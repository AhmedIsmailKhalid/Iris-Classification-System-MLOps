...
def retrain_model(min_samples: int = 30, force: bool = False) -> bool:
    ...
    if len(new_data) < min_samples and not force:
        logger.info(
            f"⚠️  Not enough samples for retraining (need {min_samples}, have {len(new_data)})"
        )
        return False
    ...
    if "species" not in new_data.columns:
        logger.warning(
            "New data missing 'species' column - cannot use for supervised retraining"
        )
    ...
    # Extract target
    combined_y = combined_X["species"].map(
        {"setosa": 0, "versicolor": 1, "virginica": 2}
    )

    logger.info(
        f"Combined dataset: {len(combined_X)} samples (original: {len(original_X)}, new: {len(new_data)})"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Retrain Iris classifier with new data"
    )
    parser.add_argument(
        "--min-samples",
        type=int,
